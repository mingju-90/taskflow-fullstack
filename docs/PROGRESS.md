...
当前本地 Node.js 为 `v22.22.3`，pnpm 为 `v10.17.0`；根与服务端声明的可安装范围为
`>=22.22.3 <23.0.0`，与 `.nvmrc` 和 CI 的 Node.js 22 保持一致。
...
- 前端依赖由根 workspace 统一安装，并通过唯一根锁文件保持依赖图可复现。
...
- Vitest 配置使用 `.mts`，与后端 ESM 运行时和 Node16 模块解析保持一致。
...
- 根目录新增 `pnpm dev` 脚本。
- 同时启动 contracts TypeScript watch、Vite 前端和 Express 后端，并按 `Ctrl+C` 统一停止。
